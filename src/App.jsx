...
const PIPELINE_STEPS = [
  {
    ...
    explanation:
      'GSRS receives the AIA cedent account claim file or event. At this point, the platform has not changed the business data. It only captures the incoming file, who uploaded it, when it arrived, and which route it came through.',
  },
  {
    id: 'register',
    stage: '2',
    ...
    explanation:
      'GSRS creates a controlled run ID and file ID so the upload can be traced end-to-end. This makes the processing auditable and allows the same run to be checked, replayed, or stopped.',
  },
  {
    id: 'bronze',
    stage: '3',
    ...
    explanation:
      'The raw AIA file is stored exactly as received. No business meaning is changed. Bronze gives GSRS an immutable audit trail and a safe replay point if later mapping or validation logic changes.',
  },
  {
    id: 'mapping',
    stage: '4',
    ...
    explanation:
      'GSRS maps AIA field names to the canonical claim model. This is where POLNUM becomes policyNumber, CLMNUM becomes claimNumber, and ORIGNCOD becomes productOrgCode. Unmapped mandatory fields are highlighted before technical checks continue.',
  },
  {
    id: 'technical',
    stage: '5',
    ...
    explanation:
      'GSRS checks whether the canonical record is technically usable. It validates mandatory fields, numeric values, date formats, duplicates, and structural quality. Records that fail are sent to technical exceptions.',
  },
  {
    id: 'silver',
    stage: '6',
    ...
    explanation:
      'GSRS writes the technically valid canonical claim into Silver. Silver stores the full cumulative operational history, using policyNumber + productOrgCode + claimNumber as the business key. This is the operational history layer that the UI can read from.',
  },
  {
    id: 'treaty',
    stage: '7',
    ...
    explanation:
      'GSRS enriches the Silver claim record using treaty document data that is not present in the cedent account file. For example, treaty section, reinsurance basis, reinsurer share, and treaty lineage are added to the claim context.',
  },
  {
    id: 'business',
    stage: '8',
    ...
    explanation:
      'GSRS checks the enriched Silver record against policy, product, benefit, treaty, and claim rules. This confirms whether the claim makes business sense before operations manage the outcome.',
  },
  {
    id: 'manage',
    stage: '9',
    ...
    explanation:
      'Operations review the claim, evidence, validations, and treaty context. The claim is routed to either the settled path or the declined path.',
  },
  {
    id: 'outcome',
    stage: '10',
    ...
    explanation:
      'GSRS captures the final claim decision. For a settlement, the settlement amount and payment date are written. For a decline, the decline reason and decision evidence are written.',
  },
  {
    id: 'silverOutcome',
    stage: '11',
    ...
    technology: 'Silver operational Delta table',
    explanation:
      'The final outcome is appended back into the Silver lifecycle, preserving the full journey from initial notification to final settlement or decline.',
  },
  {
    id: 'gold',
    stage: '12',
    ...
    explanation:
      'GSRS creates curated data products for reporting, MI, and analytics. Gold is not the main operational history store. Silver remains the source for operational claim history.',
  },
];
...
function executePipeline(outcome) {
  ...
  addSnapshot('register', {
    ...
    action: [
      'Create run ID',
      'Create file ID',
      'Set pipeline status to RECEIVED',
    ],
  ...
  addSnapshot('bronze', {
    ...
    written: [
      'bronze_aia_claim_raw.rawPayload',
      'sourceFileName',
      'ingestionRunId',
    ],
  ...
  const unmappedFields = Object.keys(raw).filter(
    (field) => !CANONICAL_MAPPING[field]
  );
  ...
  addSnapshot('mapping', {
    incoming: [
      ...
      'GSRS canonical claim model',
    ],
    action: [
      'Map POLNUM to policyNumber',
      'Map CLMNUM to claimNumber',
      'Map ORIGNCOD to productOrgCode',
      'Create canonical claim record',
    ],
    validation: [
      'Mandatory mappings found',
      `Unmapped fields: ${unmappedFields.length || 0}`,
    ],
    written: [
  ...
  if (Number.isNaN(Number(c.settlementAmount)))
    technicalErrors.push('settlementAmount is not numeric');
  if (c.paymentDate && !/^\d{4}-\d{2}-\d{2}$/.test(c.paymentDate))
    technicalErrors.push('paymentDate is not YYYY-MM-DD');
  ...
  addSnapshot('technical', {
    ...
    validation: technicalErrors.length
      ? technicalErrors
      : ['All technical checks passed'],
    written: ['technical_validation_result', 'technical_error_code if failed'],
    result: technicalErrors.length
      ? 'Record blocked by technical exception.'
      : 'Record can be written to Silver.',
  });
  ...
  addSnapshot('silver', {
    ...
    action: [
      'Create business key',
      'Append reporting period',
      'Preserve prior states',
      'Create Silver lifecycle row',
    ],
    validation: [
  ...
  const treatyMatch =
    TREATY_DOCUMENT.productOrgCode === silverClaim.productOrgCode &&
    TREATY_DOCUMENT.benefitCode === silverClaim.benefitCode;
  state.enrichment = {
  ...
  if (treatyMatch) {
    ...
    state.exceptions.push({
      type: 'ENRICHMENT',
      errors: ['Treaty not matched'],
    });
  }
  addSnapshot('treaty', {
    incoming: [
      'Silver claim lifecycle record',
      'Treaty document data',
      'productOrgCode',
      'benefitCode',
    ],
    action: [
      'Match claim to treaty document',
      'Add treaty section',
      'Add reinsurance basis',
      'Create treaty lineage',
    ],
    validation: treatyMatch
      ? ['Treaty document matched', 'Benefit mapping valid', 'Lineage captured']
      : ['Treaty not matched'],
    written: [
      'treatyReference',
      'treatySection',
      'reinsuranceBasis',
      'treatyDocumentLineage',
    ],
    result: treatyMatch
      ? 'Silver record has been enriched with treaty context.'
      : 'Claim sent to enrichment exception queue.',
  });

  const businessErrors = [];
  const enriched = state.silver[0];
  if (!enriched.treatyReference)
    businessErrors.push('Missing treaty reference');
  if (enriched.settlementAmount > enriched.sumAssured)
    businessErrors.push('Settlement exceeds sum assured');
  if (enriched.cededAmount > enriched.netAmountAtRisk)
    businessErrors.push('Ceded amount exceeds NAR');
  ...
  addSnapshot('business', {
    incoming: [
      'treaty-enriched Silver claim record',
      'policy/product/treaty rules',
    ],
    action: [
      'Check treaty context exists',
      'Check claim amount vs sum assured',
      'Check ceding amount vs NAR',
    ],
    validation: businessErrors.length
      ? businessErrors
      : ['All business checks passed'],
    written: ['business_validation_result', 'business_ready_flag'],
    result: businessErrors.length
      ? 'Claim requires business review.'
      : 'Claim can be managed by operations.',
  });
  ...
  addSnapshot('manage', {
    incoming: [
      'business-valid Silver record',
      'validation evidence',
      'treaty context',
    ],
    action: [
      'Claims operations review record',
      'Confirm evidence',
      'Route to selected outcome path',
    ],
    validation: ['Decision maker authorised', 'Required evidence present'],
    written: ['claim_decision_route', 'review_notes', 'approval_status'],
    result: `Claim routed to ${
      outcome === 'settled' ? 'settlement' : 'decline'
    } path.`,
  });
  ...
  addSnapshot('outcome', {
    ...
    action:
      outcome === 'settled'
        ? [
            'Set claim status to SETTLED',
            'Capture payment date',
            'Calculate recovery',
          ]
        : [
            'Set claim status to DECLINED',
            'Capture decline reason',
            'Attach review evidence',
          ],
    validation:
      outcome === 'settled'
        ? ['Settlement amount exists', 'Payment date exists']
        : ['Decline reason exists', 'Reviewer recorded'],
  ...
  state.silver[0] = {
    ...
    history: [
      ...
      {
        period: '2026-03',
        status: state.outcomeRecord.claimStatus,
        amount: state.outcomeRecord.settlementAmount || null,
        date:
          state.outcomeRecord.paymentDate || state.outcomeRecord.decisionDate,
      },
    ],
  };
  addSnapshot('silverOutcome', {
    ...
    action: [
      'Append final outcome to lifecycle',
      'Preserve previous states',
      'Make full history visible',
    ],
    validation: ['Lifecycle sequence valid', 'Lineage complete'],
    written: [
      'silver_claim_lifecycle.finalOutcome',
      'currentClaimStatus',
      'history[]',
    ],
  ...
  addSnapshot('gold', {
    ...
    action:
      outcome === 'settled'
        ? [
            'Create paid claims measure',
            'Create recovery measure',
            'Publish settlement data product',
          ]
        : [
            'Create declined claims measure',
            'Create decline reason analysis',
            'Publish decline data product',
          ],
...
function recordTableFromObject(object, preferredColumns = []) {
  ...
  const columns = preferredColumns.length
    ? preferredColumns.filter((column) =>
        Object.prototype.hasOwnProperty.call(object, column)
      )
    ...
    return [
      {
        ...
        rows: state.sourceFile.rows.map((row) =>
          Object.keys(state.sourceFile.rows[0]).map((column) =>
      ...
      },
      {
        ...
        rows: [
          [
            ...
            state.sourceFile.channel,
            state.sourceFile.cedent,
            state.sourceFile.reportingPeriod,
            state.sourceFile.uploadedBy,
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        ...recordTableFromObject(state.control, [
          'runId',
          'fileId',
          'status',
          'createdAt',
    ...
    return [
      {
        ...
        rows: Object.entries(CANONICAL_MAPPING).map(
  ...
  if (stepId === 'technical') {
    const checks = [
      [
        ...
        state.canonicalRecords[0]?.policyNumber ? 'PASS' : 'FAIL',
      ],
      [
        'claimNumber',
        'Mandatory field present',
        state.canonicalRecords[0]?.claimNumber ? 'PASS' : 'FAIL',
      ],
      [
        'productOrgCode',
        'Mandatory field present',
        state.canonicalRecords[0]?.productOrgCode ? 'PASS' : 'FAIL',
      ],
      [
        'settlementAmount',
        'Numeric value',
        !Number.isNaN(Number(state.canonicalRecords[0]?.settlementAmount))
          ? 'PASS'
          : 'FAIL',
      ],
      [
        ...
        /^\d{4}-\d{2}-\d{2}$/.test(state.canonicalRecords[0]?.paymentDate || '')
          ? 'PASS'
          : 'FAIL',
      ],
    ];
    return [
      ...
      },
      {
        ...
        rows: [
          [
            state.technicalValidation.status,
            state.technicalValidation.checkedAt,
            state.technicalValidation.errors.join('; ') || 'None',
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        rows: [
          [
            ...
            silver.policyNumber,
            silver.claimNumber,
            silver.productOrgCode,
            silver.lifecycleStatus,
            silver.technicalValidationStatus,
          ],
        ],
      },
      {
    ...
    return [
      {
        ...
        rows: [
          [
            silver.treatyReference,
            silver.treatySection,
            silver.reinsuranceBasis,
            formatValue(silver.reinsurerSharePct),
            silver.recoveryBasis,
            silver.treatyDocumentLineage,
          ],
        ],
      },
      {
        ...
        rows: [
          [
            ...
            state.enrichment.treatyDocument || 'Not matched',
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        rows: [
          [
            'Treaty reference exists',
            enriched.treatyReference || '',
            enriched.treatyReference ? 'PASS' : 'FAIL',
          ],
          [
            'Settlement amount <= sum assured',
            `${enriched.settlementAmount} <= ${enriched.sumAssured}`,
            enriched.settlementAmount <= enriched.sumAssured ? 'PASS' : 'FAIL',
          ],
          [
            'Ceded amount <= net amount at risk',
            `${enriched.cededAmount} <= ${enriched.netAmountAtRisk}`,
            enriched.cededAmount <= enriched.netAmountAtRisk ? 'PASS' : 'FAIL',
          ],
        ],
      },
      {
        ...
        rows: [
          [
            state.businessValidation.status,
            state.businessValidation.checkedAt,
            state.businessValidation.errors.join('; ') || 'None',
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        rows: [
          [
            state.claimManagement.workQueue,
            state.claimManagement.assignedTo,
            state.claimManagement.route,
            formatValue(state.claimManagement.decisionPending),
            state.claimManagement.reviewedAt,
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        rows: [
          Object.keys(state.outcomeRecord).map((column) =>
            formatValue(state.outcomeRecord[column])
          ),
        ],
      },
    ];
    ...
    return [
      ...
      },
      {
        ...
        rows: [
          [
            silver.businessKey,
            silver.currentClaimStatus,
            silver.finalOutcome.claimStatus,
          ],
        ],
      },
    ];
    ...
    return [
      {
        ...
        rows: [
          ...
          ...Object.entries(state.gold.measures).map(([key, value]) => [
            state.gold.table,
            key,
            formatValue(value),
          ]),
          ...Object.entries(state.gold.dimensions).map(([key, value]) => [
            state.gold.table,
            key,
            formatValue(value),
          ]),
...
function App() {
  ...
  const selectedSnapshot =
    snapshots.find((item) => item.stepId === selectedStepId) || snapshots[0];
    ...
    <div className="app">
      <aside className="left-nav">
        ...
        <div className="route-toggle">
          <button
            className={outcome === 'settled' ? 'active settled' : ''}
            onClick={() => setOutcome('settled')}
          >
            Settled route
          </button>
          <button
            className={outcome === 'declined' ? 'active declined' : ''}
      ...
      <main className="right-content">
        <section className="top-detail">
          <div className="panel-title">
            <div>
              ...
              <h2>
                {selectedStep.stage}. {selectedStep.title}
              </h2>
            </div>
            <div className={`status-chip ${outcome}`}>
              {outcome === 'settled' ? 'Settlement path' : 'Decline path'}
            </div>
          </div>
          ...
          <div className="step-summary">
            <div>
              ...
              <strong>{selectedStep.technology}</strong>
            </div>
            <div>
              <span>Layer</span>
              <strong>{selectedStep.layer}</strong>
            </div>
            <div>
              <span>What happens</span>
              <strong>{selectedStep.explanation}</strong>
            </div>
          </div>
          ...
          <div className="movement-grid">
            <MiniList
              ...
              items={selectedSnapshot.movement.incoming}
              tone="incoming"
            />
            <MiniList
              title="Backend action"
              items={selectedSnapshot.movement.action}
              tone="action"
            />
            <MiniList
              title="Validation checks"
              items={selectedSnapshot.movement.validation}
              tone="validation"
            />
            <MiniList
              title="Data written / changed"
              items={selectedSnapshot.movement.written}
              tone="written"
            />
          </div>
        </section>
        ...
        <section className="bottom-table">
          <div className="panel-title table-title">
            ...
            <div className="neutral-chip">
              Silver for operations / Gold for curated products
            </div>
          </div>
